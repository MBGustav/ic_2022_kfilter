...
struct matrix* static_attitude(IMU_data *imu_data, MAG_data *mag_data){
    ...
    __data_type *pAcc = acc->elements;
    __data_type *pMag = mag_data->mag->elements;
    __data_type *pMag_tim = mag_data->tim->elements;
    __data_type *pImu_tim = imu_data->tim->elements; 
    ...
    for(int i = 0; i < tam; i++){
        if(pMag_tim[j] < pImu_tim[i] || FLT_EQ(pMag_tim[j], pImu_tim[i])){ //fix: usar "<" em vez de "=<"
            ...
            //calculo -> roll
            roll  = atan_2d(pAcc[1], pAcc[2])*(180/PI); /*roll = euler[j,1]*/
            
            //calculo -> pitch
            pitch = atan_2d(-pAcc[0], sqrt(pAcc[1]*pAcc[1] + pAcc[2]*pAcc[2]))*(180/PI);
            
            //calculo -> yaw
            num = pMag[3*(j)+2] * sin_d(roll)  - pMag[3*j+1] * cos_d(roll);
            den = pMag[3*(j)] * cos_d(pitch) + pMag[3*j+1] * sin_d(pitch) * sin_d(roll) + pMag[3*j+2] * sin_d(pitch) * cos_d(roll);
            yaw = atan_2d(num, den) * (180/PI) - 2*constant_beta;
