...
dataset* load_dataset()
{
    ...
    out->rpy = import_RPY();
        ...
        // Ajuste de t0
    __data_type t0 = out->imu->tim->elements[0];
...
GPS_data *import_GPS()
{
    ...
    __data_type t0 = data->tim->elements[0];
...
__data_type local_gravity(IMU_data *imu_data, __data_type initial_time){
    
    __data_type x2,y2,z2;
    __data_type gravity = 0.0f;
    int count=0, j=1, nD=3;
    __data_type *p_imu = imu_data->acc->elements; 
    __data_type *p_time= imu_data->tim->elements;
...
__data_type local_magnetic(MAG_data *mag_data,__data_type initial_time){
    __data_type x2,y2,z2;
    __data_type magnetic = 0.0f;
    int count=0, j=1, nD=3;
    __data_type *p_mag = mag_data->mag->elements; 
    __data_type *p_time= mag_data->tim->elements;
