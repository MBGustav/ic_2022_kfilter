...
void propagation(state_space *State, struct matrix* omega_fil,struct matrix* acc_fil, 
                 __data_type time, struct matrix* ge, state_space *Prop){
    ...
    __data_type *pw, *p_Omega;
    __data_type nw = 0.0f; 
    __data_type w1, w2,w3;
...
// ====================================================
    ...
    __data_type *p_lg = sys_out->lambda_gyr->elements;
    __data_type *p_la = sys_out->lambda_acc->elements;
...
void state_equations(sys_param *sys, state_space *prop, __data_type time){
    int nv = 3,
        nx = 15,
        nw = 12;
...
void data_reliable(int last_mag,bool ava_mag,state_space* prop,MAG_data *mag_data,__data_type constant_gravity,__data_type constant_magnetic,bool *rel_acc,bool *rel_mag){

    __data_type rho_acc = 0.05;
    __data_type rho_mag = 1000000.0f;

    // % Teste de influência de aceleração cinemática na medida do acelerômetro    
    __data_type lim_acc = norm_m(prop->acc) / constant_gravity;
    __data_type lim_mag;
