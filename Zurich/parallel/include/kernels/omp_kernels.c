...
inline void omp_matmul(matrix *A, matrix *B, matrix *C)
{
    ...
    pB = B->elements;
    pC = C->elements;
    __data_type* pBt = (__data_type*) malloc(nrB * ncB * sizeof(__data_type));

    int ib = 2;
    int kb = 2;
    __data_type acc00, acc01, acc10, acc11;
    #pragma omp parallel
    {
...
*/
    ...
    pB = B->elements;
    pC = C->elements;
    __data_type *pAB = (__data_type*) malloc( nrA * ncB * sizeof(__data_type));
...
inline void omp_MultAcc(matrix *A, matrix *B, matrix *C, __data_type scal, matrix *out){
/*

*/
    __data_type *pA, *pB, *pC, *pout, aux;
    int nrA, nrB, ncA, ncB, ncC, nrC;
    int i, j, cont;
    nrA = A->n_row;
    ncA = A->n_col;
