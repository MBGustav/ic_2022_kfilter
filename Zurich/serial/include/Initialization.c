...
bool LessThanOrEq(double x, double y){
    const double epsilon = 0.000000000000001f;
                         ...
                         int k, int *last_val, int max_idx)
{
    ...
    //parameters
    double *dt_tim = data_tim->elements;
    double *imu_tim = imu_time->elements;
                ...
                data0 *data_0) /*saida de dados*/
{
    ...
    while(LessThanOrEq(mag_data->tim->elements[idx], (double) initial_time )){
        //if(idx < max_bound)
            data_0->last_mag = idx++;
        //else break;
    }
    ...
    double roll = angle2r(data_0->rpy->elements[0]);
    double pitch= angle2r(data_0->rpy->elements[1]);
    double yaw  = angle2r(data_0->rpy->elements[2]);
    
    angle2quat(yaw, pitch, roll, data_0->qtn);
    angle2dcm (yaw, pitch, roll, data_0->dcm); 
    
    // data_0->bias_gyr = (double*) malloc(sizeof(double)*15); //conforme definido em MATLAB
