...
struct matrix *matrix_m(int n_row, int n_col) {
        ...
        M->elements = (double*) (double*) calloc(n_row * n_col, sizeof(double));
...
void rnd_matrix(struct matrix* A)
{
        ...
        for(int j =0; j < A->n_col; j++)
                A->elements[i* ncA + j] = (double) (rand()%100);
} 
// Print a matrix
void print_m(struct matrix *M , double a) {
        double *p;
        int nr, nc;
        int i,j;
...
void mat_cpy(struct matrix *src, struct matrix* dst){
        ...
        double *psrc = src->elements;
        double *pdst = dst->elements; 
...
void mat_cpyRow(struct matrix *A_src, int r_src ,struct matrix* B_dst, int r_dst){
        ...
        double *p_dst = B_dst->elements;
        double *p_src = A_src->elements;
...
void geodetic2ecef(struct matrix* llh, struct matrix* xyz){
        double lat = angle2r(llh->elements[0]);
        double lon = angle2r(llh->elements[1]);
        double h = llh->elements[2];
...
void ecef2ned(struct matrix* pe, struct matrix *llh0, struct matrix *pt){
        double lat0= angle2r(llh0->elements[0]);
        double lon0= angle2r(llh0->elements[1]);
        double h0  = llh0->elements[2];

        struct matrix *RTE = matrix_m(3,3);
        struct matrix *pe0 = matrix_m(3,1); 
        struct matrix *aux = matrix_m(3,1);
        double *pRTE  = RTE->elements;
...
struct matrix* read_matrix(char* name)
{
 ...
 bool matrix_isEqual(struct matrix *A, struct matrix *B){
        ...
        int nrA = A->n_row,
            ...
            ncB = B->n_col,
            size= nrA * ncA;
        double *pA = A->elements;
        double *pB = B->elements;
        if(nrA != nrB || ncA != ncB){
