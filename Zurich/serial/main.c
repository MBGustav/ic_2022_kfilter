...
void display_state(state_space *state)
{
    debug(state->bias_gyr, 1000);
    debug(state->omegag, 1000);
    debug(state->bias_acc, 1000);
    debug(state->acc, 1000);
    debug(state->qtn, 1000);
    debug(state->vel, 1000);
    debug(state->pos, 1000);
    // if((spc->P != NULL)) debug(spc->P, 1000);
}


void display_sys(sys_param *s)
{
    debug(s->F, 1);
    debug(s->G, 1);
    debug(s->H, 1);
    debug(s->lambda_gyr, 1);
    debug(s->lambda_acc, 1);
    debug(s->R, 1);
    debug(s->Q, 1);
}
...
int main(){
    ...
    #ifdef _USE_AVX
    timer_setconf("fun_time_avx.out", &timer);
    timer_setconf("time_avx.out", &general_timer);
    #elif _USE_OMP
    timer_setconf("fun_time_omp.out", &timer);
    timer_setconf("time_omp.out", &general_timer);
    #else
    timer_setconf("fun_time.out", &timer);
    timer_setconf("time.out", &general_timer);
    #endif
    //testando com entradas do matlab
    // struct matrix *mlab_gps     = read_matrix("matrices/dbg_last_g_mtb.csv");
    // struct matrix *mlab_mag     = read_matrix("matrices/dbg_last_m_mtb.csv");
    // struct matrix *mlab_rpy     = read_matrix("matrices/dbg_last_r_mtb.csv");
    // struct matrix *mlab_ava_gps = read_matrix("matrices/dbg_ava_g_mtb.csv");
    // struct matrix *mlab_ava_mag = read_matrix("matrices/dbg_ava_m_mtb.csv");
    // struct matrix *mlab_ava_rpy = read_matrix("matrices/dbg_ava_r_mtb.csv");
    ...
    //matrizes auxiliares
    struct matrix *sys_Ft   = matrix_m(nx, nx);
    struct matrix *sys_Gt   = matrix_m(12, nx);
    struct matrix *aux_FP   = matrix_m(nx, nx);
    struct matrix *aux_FPF  = matrix_m(nx, nx);
    struct matrix *aux_GQ   = matrix_m(nx, 12);
    struct matrix *aux_GQG  = matrix_m(nx, nx);
    ...
    for(int k= data_0->index+1; k < max_len; k++, idx++){
        ...
        if(ava_mag){
            mag->elements[0] = T2G * mag_data->mag->elements[last_mag*3 + 0]  - 0;
            mag->elements[1] = T2G * mag_data->mag->elements[last_mag*3 + 1]  - 0;
        ...
        // aux_FPF = F x P x F'
        times_m(sys->F, P, aux_FP);      // aux_FP  = F * P * F' (12 x 12)
        transp_m(sys->F, sys_Ft);
        

        times_m(aux_FP, sys_Ft, aux_FPF); // aux_FPF  = aux_FP * F' (12 x 12)
        

        // aux_GQG = G x Q x G'
        times_m(sys->G, sys->Q, aux_GQ); // aux_GQ  = G * Q | (15 x 12)
        transp_m(sys->G, sys_Gt);
        times_m(aux_GQ, sys_Gt, aux_GQG);// aux_GQG = G * Q * G' 

        sum_m(aux_FPF, aux_GQG, prop->P); 
        debug(prop->P, 1000);
