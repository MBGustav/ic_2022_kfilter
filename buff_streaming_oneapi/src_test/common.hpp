...
// version of memcpy.
template<typename T>
void threaded_consumer(T* dst, const T* src, size_t count, int num_threads) {
  if (num_threads == 1) {
...
template<typename T>
void threaded_producer(T* dst, const T* src, size_t count, int num_threads) {
  if (num_threads == 1) {
    // if the number of threads is 1, just do a regular memcpy()
    memcpy(dst, src, count * sizeof(T));
  } else {
...
void ProducerFunction(T* dst, const T* src, size_t count, int num_threads) {
  // CHANGE: Mudança para multiplicação scalar 
  threaded_producer(dst, src, count, num_threads);
}
...
void ConsumerFunction(T* dst, const T* src, size_t count, int num_threads) {
  threaded_consumer(dst, src, count, num_threads);
}
