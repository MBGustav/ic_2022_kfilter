  ...
  auto kernel_event = q.single_task<APIKernel>([=] {
    ...
    for (size_t i = 0; i < total_count; i++) {
      ...
      for(int j=0;j<data.size(); j++)
        data[j] = mult_scalar*data[j] + add_scalar;
...
bool DoWorkAPI(queue& q, size_t buffers, size_t buffer_count, size_t reps,
  ...
  // CHANGE: vetor com numeros de [0 .. size].
  std::generate_n(in_stream.begin(), total_count, [] 
      {static int data_i=0; return data_i++; });


  std::cout << "Showing the first 8 values:\n" << std::endl;
  for (size_t sz = 0; sz < 3; sz++){
    for (size_t i = 0; i < 8; i++)  
      std::cout << in_stream[i][sz] << " ";
    std::cout << "\n";
  }
  ...
  for (size_t i = 0; i < iterations; i++) {
    ...
    //                             in_stream.data());
    std::cout << "Showing the first 8 values from out_stream:\n" << std::endl;
    for (size_t sz = 0; sz < 3; sz++){
      for (size_t i = 0; i < 8; i++)  
        std::cout << out_stream[i][sz] << " ";
      std::cout << "\n";
    }
