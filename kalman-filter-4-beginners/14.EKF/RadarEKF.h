...
// }// end hx
void Hjacob(sycl::queue &q, DATA_TYPE *H, DATA_TYPE *xhat) {
    // Create a queue to execute the kernel
    // queue q(default_selector{});

    // Allocate memory for input and output on the device
    buffer<DATA_TYPE, 1> xhat_buf(xhat, range<1>{3});
    buffer<DATA_TYPE, 1> H_buf(H, range<1>{3});

    // Submit a command group to the queue
    q.submit([&](handler &h) {
        // Accessors for the buffers
        auto xhat_acc = xhat_buf.get_access<access::mode::read>(h);
        auto H_acc = H_buf.get_access<access::mode::write>(h);

        // Kernel
        h.parallel_for<class HjacobKernel>(range<1>{1}, [=](id<1> idx) {
            DATA_TYPE x1 = xhat_acc[0];
            DATA_TYPE x3 = xhat_acc[2];

            DATA_TYPE denominator = sqrt(pow(x1, 2.0) + pow(x3, 2.0));

            H_acc[0] = x1 / denominator;
            H_acc[1] = 0.0;
            H_acc[2] = x3 / denominator;
        });
    }).wait(); // Wait for the kernel to finish
}


void Hx(sycl::queue &q, DATA_TYPE *zp, DATA_TYPE *xhat) {
    // Create a queue to execute the kernel
    // queue q(default_selector{});

    // Allocate memory for input and output on the device
    buffer<DATA_TYPE, 1> xhat_buf(xhat, range<1>{3});
    buffer<DATA_TYPE, 1> zp_buf(zp, range<1>{1});

    // Submit a command group to the queue
    q.submit([&](handler &h) {
        // Accessors for the buffers
        auto xhat_acc = xhat_buf.get_access<access::mode::read>(h);
        auto zp_acc = zp_buf.get_access<access::mode::write>(h);

        // Kernel
        h.parallel_for<class HxKernel>(range<1>{1}, [=](id<1> idx) {
            DATA_TYPE x1 = xhat_acc[0];
            DATA_TYPE x3 = xhat_acc[2];

            zp_acc[0] = sqrt(pow(x1, 2.0) + pow(x3, 2.0));
        });
    }).wait(); // Wait for the kernel to finish
}
