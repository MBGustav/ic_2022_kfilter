...
int main(int argc, char **argv){
    ...
    auto async_handler = [](sycl::exception_list exceptions) {
        for (std::exception_ptr const &e : exceptions) {
            try {
                ...
                cout << "Caught asynchronous SYCL exception: " << e.what() << std::endl;
            }
        }
    };
    ...
    try {
        ...
        cout << "Average time execution(ms): " << std::setw(2) << (double) duration_cast<milliseconds>(end - beg).count()/kIterations << "\n"; 
